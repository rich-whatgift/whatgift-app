...
class WhatGiftSupabaseUser extends BaseAuthUser {
  WhatGiftSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> whatGiftSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = WhatGiftSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
